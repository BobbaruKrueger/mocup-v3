...
//	$query		= "SELECT format_id, nume, datap, headline, maintext, imglink, linkdescription, format FROM ads WHERE nume='$numeWho' ORDER BY format_id DESC";
	...
	$domain		= 'http://localhost/mocup3.0';
						...
						<div class="col-12 col-lg-6">
							<div id="formW" class="formW">
								<?php 
									...
									$who		= ( empty($_GET['who']) ? $_SESSION['username'] : $_GET['who'] );
							  ...
							  <?php  
								if($response) {
									...
									while($row = mysqli_fetch_array($response)){
										...
										$sharelink = $domain.'/index.php?mainttl='.$row['headline'].'&maintxt='.$row['maintext'].'&imagsrc='.$row['imglink'].'&lnkdsc='.$row['linkdescription'].'&who='.$row['nume'].'&select_format='.$row['format'];
							...
							<?php 
								...
								if ( isset( $_POST['delTable'] ) ){
									...
									if ( $numeWho == 'admin' ) {
										...
										$sqlDt.= "ALTER TABLE ads AUTO_INCREMENT=1;";	
									} else {
										$sqlDt = "DELETE FROM ads WHERE nume='$numeWho'";	
									}
									
									if ($dbc->multi_query($sqlDt) === TRUE) {
